...
export const NavBar: React.FC = props => {
    ...
    const bucketItemsLength = useMemo(() => (
        bucketItems.reduce((prev, bucketItem) => {
            return prev + bucketItem.count;
        }, 0)
    ), [bucketItems]);

    //Обрабработчик события наведения мыши на кнопку корзины
    const onMouseEnterOnBucketHandler = useCallback((e: React.MouseEvent<HTMLAnchorElement>) => {
            ...
            <nav className="navbar container">
                <ul className="navbar__list">
                    {navbar.map((navbarItem, index) =>
                        <li key={index} className="navbar__item">{navbarItem.text}</li>)}
                </ul>
                <div className="navbar__bucket">
                    <NavLink to={'/bucket'} onMouseEnter={onMouseEnterOnBucketHandler}>
